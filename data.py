...
def createGraph(sequence,selfConn=True,thres=0.4,scaledWeights=True):
    ...
    _corrMat = np.zeros((_nNodes,_nNodes))   ## correlation matrix of sequences  [node0[0],       node1[0],       ..., nodeN]
    for _m in range(_nNodes):                ##                                  [                   ...                    ]  
        for _n in range(_nNodes):            ##                                  [node0[_length], node1[_length], ..., nodeN]   
            _mean1,_std1 = np.mean(sequence[:,_m]),np.std(sequence[:,_m])
            _mean2,_std2 = np.mean(sequence[:,_n]),np.std(sequence[:,_n])
            _corrMat[_m,_n] = np.correlate(sequence[:,_m]-_mean1,sequence[:,_n]-_mean2)/(_std1*_std2*_length)
    _corrMat[_corrMat<thres] = 0.    ### cut away links with<threshold
    _scale = np.mean(np.sum(_corrMat,axis=1))     
    if scaledWeights: _corrMat /= _scale
    _edgeIdx,_edgeAttr = utils.dense_to_sparse(torch.DoubleTensor(_corrMat))
    if not selfConn: _edgeIdx,_edgeAttr = utils.remove_self_loops(_edgeIdx,_edgeAttr)
...
class FTGenerator(object):
    ...
    def _extractMetadata(self):
        ...
        _minState[-1], _maxState[-1] = 0., 1. # Not normalizing Tmax classification
        ...
        _meanState[-1], _stdState[-1] = 0., 1. # Not normalizing Tmax classification
    ...
    def _constructRawData(self):
        ...
        _rawNew[:,:,:-1] = _raw.copy()
        _rawNew[:,:,-1] = _oni.reshape(-1,1)    ###_oni 1D array of size nSteps this broadcasts _oni to all nNodes
        _raw = _rawNew.copy()        
    ...
    def _createHistFT(self):
        ...
        if self.norm == "MinMax":
            ...
            _normData = MeanStdNorm(_rawData,self.metadata['mean'],self.metadata['std'])
        else:
            print ("Data is not normalized.")
            _normData = _rawData

        ## create edges
        _edgeIdx, _edgeAttr = createGraph(_rawData[:,:,self.graphCorrIdx],self.selfConn,self.thres,self.scaledWeights)
        ...
        _nFeatures = ((self.Cin+self.Cout)*self.nHistYear+self.Cin)*len(self.featureIdx)
        ...
        for _n in range(_nSteps):
            _window = [np.arange(_n+_m*365,_n+_m*365+(self.Cin+self.Cout)) for _m in range(self.nHistYear)]
            _window = np.array(_window).flatten()
            _window = np.hstack([_window,np.arange(_startIdx+_n,_startIdx+_n+self.Cin)])
            _feature = _normData[_window]
            _feature = np.transpose(_feature[:,:,self.featureIdx],(1,0,2)).reshape(_nNodes,-1)
            _target = _normData[_startIdx+_n+self.Cin:_startIdx+_n+self.Cin+self.Cout,:,0].T
    ...
    def _createFT(self):
        ...
        ## create edges
        _edgeIdx, _edgeAttr = createGraph(_rawData[:,:,self.graphCorrIdx],self.selfConn,self.thres)
    ...
    def createDataset(self):
        _Dataset = []
        _FT,_TG,_edgeIdx,_edgeAttr = self._createFT() if self.nHistYear == 0 else self._createHistFT()
        for _n in range(len(_FT)):
            _Dataset.append(Data(
                x=torch.DoubleTensor(_FT[_n]),
                y=torch.LongTensor(_TG[_n]),
                edge_index = _edgeIdx,
                edge_attr = _edgeAttr).to(self.device))
