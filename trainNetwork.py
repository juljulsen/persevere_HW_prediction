...
flags.DEFINE_integer('graphCorrIdx', int(7), help='Use which feature to create graph correlation')
...
flags.DEFINE_bool('selfConn', True, help='Whether to keep self-connection in graph or not.')
flags.DEFINE_bool('Tdiff', False, help='Whether to use Tdiff target or not.')
flags.DEFINE_integer('nHistYear', 0, help='Number of previous year data to include as features')
...
def main(_):
    ...
    if os.path.exists(metaPath):
        with open(metaPath,'rb') as f: metadata = pickle.load(f)
    ...
    trainSet, validSet, testSet = Dataset[:trainSetLength],Dataset[trainSetLength:trainSetLength+validSetLength],Dataset[trainSetLength+validSetLength:]
    trainLoader = batchLoader(trainSet,batch_size=FLAGS.batch_size,shuffle=FLAGS.shuffleBatch)
    ...
    if FLAGS.saveModel:
        ...
        with open(paraPath, 'wb') as f: pickle.dump([modelPara,paraDict], f)
    ...
    for epoch in range(EPOCH,nEpoch):
        ...
        loss,accuTrain,[recallTrain,precTrain,f1Train] = network.train(model,opt,trainLoader,nNodes,scheduler,nClass,lossFn,lossFnWeights)
        history[epoch] = loss
        timer = time.time() - timeStart
        if not printOut:
            ## Perform validation and test
            lossValid,accuValid,[recallValid,precValid,f1Valid] = network.oneStepEval(model,validLoader,nNodes,nClass,lossFn,lossFnWeights)
            lossTest,accuTest,[recallTest,precTest,f1Test] = network.oneStepEval(model,testLoader,nNodes,nClass,lossFn,lossFnWeights)
            print ("LR =",scheduler.get_last_lr())
            print(f"Iter {epoch+1} training loss: {loss}, validation loss: {lossValid}, test loss: {lossTest} time:{timer:1.2f}")
            print(f"Mean training accuracy {accuTrain*100:1.2f}%, mean validation accuracy {accuValid*100:1.2f}%, mean test accuracy {accuTest*100:1.2f}%")
            print(f"Train recall: {recallTrain*100:1.2f}%. Validation recall: {recallValid*100:1.2f}%, Test recall: {recallTest*100:1.2f}%")
            print(f"Train precision: {precTrain*100:1.2f}%. Validation precision: {precValid*100:1.2f}%, Test precision: {precTest*100:1.2f}%")
            print(f"Train f1: {f1Train*100:1.2f}%. Validation f1: {f1Valid*100:1.2f}%, Test f1: {f1Test*100:1.2f}%")
            
            with open('log','a') as f:
                print ("LR =",scheduler.get_last_lr(),file=f)
                print(f"Iter {epoch+1} training loss: {loss}, validation loss: {lossValid}, test loss: {lossTest} time:{timer:1.2f}",file=f)
                print(f"Mean training accuracy {accuTrain*100:1.2f}%, mean validation accuracy {accuValid*100:1.2f}%, mean test accuracy {accuTest*100:1.2f}%",file=f)
                print(f"Train recall: {recallTrain*100:1.2f}%. Validation recall: {recallValid*100:1.2f}%, Test recall: {recallTest*100:1.2f}%",file=f)
                print(f"Train precision: {precTrain*100:1.2f}%. Validation precision: {precValid*100:1.2f}%, Test precision: {precTest*100:1.2f}%",file=f)
                print(f"Train f1: {f1Train*100:1.2f}%. Validation f1: {f1Valid*100:1.2f}%, Test f1: {f1Test*100:1.2f}%",file=f)
                print ("",file=f)
            torch.save({
                        'epoch':            epoch,
